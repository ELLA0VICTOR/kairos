...
export function time(ts:number,zone='America/New_York',pattern='EEE HH:mm'):string {
 const key=ts+'|'+zone+'|'+pattern,cached=timeLabels.get(key);if(cached!==undefined)return cached;
 const label=formatInTimeZone(ts,zone,pattern);if(timeLabels.size>=512)timeLabels.clear();timeLabels.set(key,label);return label;
}
