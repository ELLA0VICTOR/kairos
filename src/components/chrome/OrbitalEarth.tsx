...
export const OrbitalEarth=memo(function OrbitalEarth(){
 ...
 const reduced=useReducedMotion(),[paused,setPaused]=useState(false),[ready,setReady]=useState(false),[activated,setActivated]=useState(false);
 const settings=useRef({paused,reduced});settings.current={paused,reduced};
 useEffect(()=>{send.current();},[paused,reduced]);
 useEffect(()=>{
  const canvas=canvasRef.current;if(!canvas)return;
  let started=false,startTimer=0;
  let disposed=false,visible=false,worker:Worker|undefined,fallback:ReturnType<typeof import('./earth-renderer').renderEarth>|undefined;
  ...
  const observer=new IntersectionObserver(([entry])=>{visible=entry?.isIntersecting??false;if(!started&&entry&&entry.intersectionRatio>=.25){started=true;setActivated(true);startTimer=window.setTimeout(start,0);}update();},{threshold:[0,.25]});observer.observe(canvas);
  ...
  return()=>{disposed=true;clearTimeout(startTimer);worker?.terminate();fallback?.dispose();resize.disconnect();observer.disconnect();canvas.removeEventListener('pointermove',move);document.removeEventListener('visibilitychange',update);send.current=()=>{};};
 },[]);

  return <div className={`orbital-earth ${activated?'earth-active':''}`}>
