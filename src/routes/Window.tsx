...
export default function Window({ intro }: {
    ...
    const { board, loading, error, retry, fallbacks } = useDesk(), [sort, setSort] = useState<Sort>('Drift'), [desc, setDesc] = useState(true), [layout, setLayout] = useState<'auto' | 'board' | 'cards'>('auto'), [query, setQuery] = useState(''), [group, setGroup] = useState<'stocks' | 'signals' | 'anchors'>('stocks');
    const stocks = board.rows.filter(r => !isAnchor(r)), signals = stocks.filter(outside), widest = [...stocks].sort((a, b) => Math.abs(b.reckoning.drift) - Math.abs(a.reckoning.drift))[0];
