  ...
  <div className="agent-thread">
   {turns.map((turn,index)=><section className="agent-turn" key={turn.id} ref={index===turns.length-1?latest:null}>
    ...
    <article className="agent-answer" data-research-note aria-busy={turn.status==='working'}>
     ...
     {turn.result&&<p className="agent-provenance" data-language-mode={turn.result.mode}>{turn.result.mode==='none'?'Engine-backed answer · built-in explanation':`Written with ${turn.result.mode==='qwen'?'Qwen':'OpenAI'} · engine-backed figures`}</p>}
