...
export async function* research(question:string,data:EngineSnapshot,options:ResearchOptions={}):AsyncGenerator<ResearchEvent>{
  ...
  try {
    ...
    while(!controller.signal.aborted){
      ...
      const iterator=client.complete({system:RESEARCH_SYSTEM,messages,tools:calls.length<6?TOOLS:undefined,maxTokens:1200,temperature:.2,signal:controller.signal})[Symbol.asyncIterator]();
      try {
        while(true){
