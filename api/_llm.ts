import { reserveLanguageBudget } from './_guards';
...
export type LlmChunk = {type:'text';delta:string}|{type:'tool';call:ToolCall}|{type:'usage';tokens:number};
export interface LlmClient {
  readonly id:'qwen'|'none';
...
export class QwenClient implements LlmClient {
  ...
  constructor(private readonly key:string,private readonly base='https://hackathon.bitgetops.com/v1',private readonly model='qwen3.8-max',private readonly transport:typeof fetch=fetch){}
  async *complete(args:Parameters<LlmClient['complete']>[0]):AsyncIterable<LlmChunk>{
    ...
    const parse=(line:string):LlmChunk[]=>{
      ...
      const message=JSON.parse(raw) as {choices?:Array<{delta?:{content?:string;tool_calls?:Array<{index:number;id?:string;function?:{name?:string;arguments?:string}}>}}>;usage?:{total_tokens:number};error?:unknown};
      ...
      if(message.usage&&Number.isFinite(message.usage.total_tokens))chunks.push({type:'usage',tokens:message.usage.total_tokens});return chunks;
    };
    try {
...
export function getLlmClient():LlmClient {
  const key=process.env.QWEN_API_KEY?.trim();if(!key)return noneClient;
  const client=new QwenClient(key,process.env.QWEN_BASE_URL,process.env.QWEN_MODEL);
  return {id:'qwen',async *complete(args){
    ...
    if(!reserveLanguageBudget(promptBytes+args.maxTokens+512))throw new LanguageBudgetPaused('Language service paused for today. Figures are unaffected.');
    yield* client.complete(args);
  }};
}
